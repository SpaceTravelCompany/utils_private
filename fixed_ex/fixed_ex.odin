...
array_equal :: proc "contextless" (
	a: $T/[$N]fixed.Fixed($Backing, $Fraction_Width),
	b: T,
) -> bool {
	#unroll for i in 0 ..< N {
...
@(require_results)
sub2 :: #force_inline proc "contextless" (
	a, b: [2]$T/fixed.Fixed($Backing, $Fraction_Width),
) -> [2]T {
...
@(require_results)
length2 :: #force_inline proc "contextless" (
	v: [2]$T/fixed.Fixed($Backing, $Fraction_Width),
) -> T {
